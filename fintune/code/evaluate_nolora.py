...
# CHECKPOINT_PATH = "/home/ps/jiaq/code/qwen2.5-14b-qlora-graph-COT3/final"

# 数据路径
BASE_PATH = "/home/ps/jiaq/InstructCom/dataset/synthetic-2000-20"  
LABEL_FILE = "/home/ps/jiaq/InstructCom/dataset/synthetic-2000-20/2000-20-node-labels.txt"
EDGE_FILE = "/home/ps/jiaq/InstructCom/dataset/synthetic-2000-20/2000-20-hyperedges.txt"
OUTPUT_PATH = "/home/ps/jiaq/InstructCom/output/synthetic-2000-20/eval_results_hypergraph_nolora.json"
LOG_FILE = "/home/ps/jiaq/InstructCom/output/synthetic-2000-20/debug_expansion_nolora.log"  # 新增：调试日志文件路径
# 推理参数
...
class HypergraphEnvironment:
    def __init__(self, base_path, label_file, edge_file):
        ...
        self.hyperedges = []  # 存储超边列表 [[1,2], [3,4,5]]
        self.node_to_edges = defaultdict(set)  # 倒排索引: node_id -> set(edge_indices)
    ...
    def _load_data(self):
        ...
        with open(self.hyperedge_path, 'r', encoding='utf-8') as file:
            for idx, line in enumerate(file):  # idx 从 0 开始作为边ID
                line = line.strip()
                if line:
...
def evaluate():
    # --- 初始化环境 ---
    # 初始化超图结构,env对象包含了这个超图数据集的label标签信息，hyperedges超边信息，true_communities真实社区
    ...
    # --- 选择测试种子 ---
    with open("/home/ps/jiaq/InstructCom/dataset/synthetic-2000-20/synthetic-dataset_community_split.json", "r", encoding="utf-8") as f:
    ...
    logger.info(f"Max community size across all communities: {max_comm_size}")
    logger.info(f"Starting Baseline (Modularity-Only) evaluation on {len(test_communities)} communities...")
    for label, nodes in tqdm(test_communities):
        ...
        for seed_node in selected_seeds:
            ...
            while len(current_community) < max_comm_size: # 将终止条件设置为社区数目当中的最大社区数目
